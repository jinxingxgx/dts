...
public class DtsAdService {
	...
	public List<DtsAd> queryIndex() {
		...
		example.createCriteria().andPositionEqualTo(1).andDeletedEqualTo(false).andEnabledEqualTo(true);
		return adMapper.selectByExample(example);
	}
	public List<DtsAd> queryIndex2() {
		DtsAdExample example = new DtsAdExample();
		example.createCriteria().andPositionEqualTo(2).andDeletedEqualTo(false).andEnabledEqualTo(true);
		return adMapper.selectByExample(example);
	}
	public List<DtsAd> queryWithPostion(Integer postion) {
		DtsAdExample example = new DtsAdExample();
		example.or().andPositionEqualTo(postion).andDeletedEqualTo(false).andEnabledEqualTo(true);
		return adMapper.selectByExample(example);
	}
