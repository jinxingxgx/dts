  ...
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" size="small" element-loading-text="正在查询中。。。" border fit highlight-current-row>
      ...
      <el-table-column align="center" label="操作" width="200" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button v-permission="['GET /admin/user/detailApprove']" v-if="scope.row.status==0 && scope.row.userLevel==2" type="primary" @click="handleDetail(scope.row)">取消司机</el-button>
          <el-button v-permission="['POST /admin/user/approveAgency']" v-else type="primary" @click="handleApproveAgency(scope.row)">成为司机</el-button>
        </template>
      </el-table-column>
    ...
    <!-- 详情对话框 -->
    <el-dialog :visible.sync="detailDialogVisible" title="司机详情" width="700">
    ...
    <!-- 代理审批 -->
    <el-dialog :visible.sync="approveDialogVisible" title="确定要将该用户设置成为出租车司机？">
      <el-form ref="approveForm" :model="approveForm" status-icon label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
        <el-form-item label="备注" prop="settlementRate">
...
<script>
import { fetchList, approveAgency, detailApprove, changeUser } from '@/api/user'
...
export default {
  ...
  methods: {
    ...
    handleDetail(row) {
	 changeUser(row.id).then(response => {
